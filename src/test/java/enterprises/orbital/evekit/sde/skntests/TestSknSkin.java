...
public class TestSknSkin extends TestSetup {
  ...
  public void testTableCount() {
    ...
    List<SknSkin> next = SknSkin.access(contid, maxresults, all, all, all, all, all, all, all);
    while (!next.isEmpty()) {
      contid += next.size();
      next = SknSkin.access(contid, maxresults, all, all, all, all, all, all, all);
    }
    Assert.assertEquals(1605, contid);
  }

  @Test
  public void testRandomElement() {
    AttributeSelector all = new AttributeSelector("{any:true}");
    List<SknSkin> next = SknSkin.access(0, 1000, new AttributeSelector("{values:[12]}"), all, all, all, all, all, all);
