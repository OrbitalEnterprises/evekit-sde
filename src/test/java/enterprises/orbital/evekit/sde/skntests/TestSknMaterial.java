...
public class TestSknMaterial extends TestSetup {
  ...
  public void testTableCount() {
    ...
    List<SknMaterial> next = SknMaterial.access(contid, maxresults, all, all, all, all, all, all, all, all);
    while (!next.isEmpty()) {
      contid += next.size();
      next = SknMaterial.access(contid, maxresults, all, all, all, all, all, all, all, all);
    }
    Assert.assertEquals(69, contid);
  }

  @Test
  public void testRandomElement() {
    AttributeSelector all = new AttributeSelector("{any:true}");
    List<SknMaterial> next = SknMaterial.access(0, 1000, new AttributeSelector("{values:[7]}"), all, all, all, all, all, all, all);
