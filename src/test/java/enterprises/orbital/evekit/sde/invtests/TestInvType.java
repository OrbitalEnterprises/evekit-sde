...
public class TestInvType extends TestSetup {
  ...
  public void testTableCount() {
    ...
    List<InvType> next = InvType.access(contid, maxresults, all, all, all, all, all, all, all, all, all, all, all, all, all, all, all, all, all, all);
    while (!next.isEmpty()) {
      contid += next.size();
      next = InvType.access(contid, maxresults, all, all, all, all, all, all, all, all, all, all, all, all, all, all, all, all, all, all);
    }
    Assert.assertEquals(29422, contid);
  }
  ...
  public void testRandomElement() {
    ...
    Assert.assertEquals(new Double(3532), random.getRadius(), 0.001);
    ...
    Assert.assertEquals(BigDecimal.ZERO.setScale(2, RoundingMode.DOWN), random.getBasePrice());
