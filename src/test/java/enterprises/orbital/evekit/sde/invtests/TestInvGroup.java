...
public class TestInvGroup extends TestSetup {
  ...
  public void testTableCount() {
    ...
    List<InvGroup> next = InvGroup.access(contid, maxresults, all, all, all, all, all, all, all, all, all, all, all, all);
    while (!next.isEmpty()) {
      contid += next.size();
      next = InvGroup.access(contid, maxresults, all, all, all, all, all, all, all, all, all, all, all, all);
    }
    Assert.assertEquals(1175, contid);
  }

  @Test
  public void testRandomElement() {
    AttributeSelector all = new AttributeSelector("{any:true}");
    List<InvGroup> next = InvGroup.access(0, 1000, new AttributeSelector("{values:[9]}"), all, all, all, all, all, all, all, all, all, all, all);
    ...
    Assert.assertNull(random.getAllowManufacture());
    Assert.assertNull(random.getAllowRecycler());
