...
public class TestInvCategory extends TestSetup {
  ...
  public void testTableCount() {
    ...
    List<InvCategory> next = InvCategory.access(contid, maxresults, all, all, all, all, all);
    while (!next.isEmpty()) {
      contid += next.size();
      next = InvCategory.access(contid, maxresults, all, all, all, all, all);
    }
  ...
  public void testRandomElement() {
    AttributeSelector all = new AttributeSelector("{any:true}");
    List<InvCategory> next = InvCategory.access(0, 1000, new AttributeSelector("{values:[4]}"), all, all, all, all);
